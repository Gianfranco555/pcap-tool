...
def correlate_icmp_errors(packets: pd.DataFrame, flows: pd.DataFrame) -> pd.DataFrame:
    ...
    flow_lookup: Dict[Tuple[Any, Any, Any, Any, Any], int] = {}
    ...
    for r in packets.itertuples(index=False):
        ...
        if proto in {"ICMP", "ICMPV6"}:
            ...
            key = (orig_dst, orig_dport, str(orig_proto).upper() if orig_proto else None, orig_ipid)
            fid = packet_map.get(key)
            if fid is not None:
                df.loc[df.flow_id == fid, "icmp_error_count"] += 1
            continue
