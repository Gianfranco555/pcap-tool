...
def handle_parse_errors(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        except (FileNotFoundError, pyshark.tshark.tshark.TSharkNotFoundException) as exc:  # pragma: no cover - runtime protection
            logger.error("File or TShark error: %s", exc)
            raise
...
class PySharkParser(BaseParser):
    ...
    def _create_capture(
        ...
        *,
        ...
        try:
            ...
            logger.error("TShark crashed while loading packets: %s", exc)
            raise RuntimeError("TShark crashed during capture load") from exc
        except (FileNotFoundError, pyshark.tshark.tshark.TSharkNotFoundException) as exc:
            logger.error("Error loading capture: %s", exc)
            raise
        except Exception:
    ...
    def _extract_tcp_data(self, ext: PacketExtractor, record: PcapRecord) -> None:
        ...
        win_val = ext.get("tcp", "window_size_value", record.frame_number)
        if win_val is None:
            win_val = ext.get("tcp", "window_size", record.frame_number)
        record.tcp_window_size = _safe_int(win_val)
        record.tcp_stream_index = _safe_int(ext.get("tcp", "stream", record.frame_number))

        mss_val = ext.get("tcp", "options_mss_val", record.frame_number)
        if mss_val is None:
            mss_val = ext.get("tcp", "mss_val", record.frame_number)
        if mss_val is not None:
            record.tcp_options_mss = _safe_int(mss_val)

        sack_perm = ext.get("tcp", "options_sack_permit", record.frame_number)
        if sack_perm is None:
            sack_perm = ext.get("tcp", "sack_perm", record.frame_number)
        if sack_perm is not None:
            record.tcp_options_sack_permitted = _safe_str_to_bool(sack_perm)

        wscale_val = ext.get("tcp", "options_wscale_val", record.frame_number)
        if wscale_val is None:
            wscale_val = ext.get("tcp", "window_scale_multiplier", record.frame_number)
        if wscale_val is not None:
            record.tcp_options_window_scale = _safe_int(wscale_val)

        flow_key = _flow_history_key(record.source_ip, record.source_port, record.destination_ip, record.destination_port)
    ...
    def parse(
        ...
        *,
        ...
        cap = self._create_capture(file_path, start=start, slice_size=slice_size)
        generated = 0
        try:
            for pkt in cap:
                yield self._packet_to_record(pkt)
                generated += 1
                if max_packets is not None and generated >= max_packets:
                    break
        finally:
            cap.close()
...
# Legacy parser generator retained for external callers relying on the old API.
# It mirrors the original monolithic implementation and will be removed in a
