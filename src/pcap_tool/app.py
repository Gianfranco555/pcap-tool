            ...
            def _guess_row(row: pd.Series) -> str:
                data = {
                    ...
                    "destination_port": row.get(
                        "destination_port",
                        row.get("dest_port", row.get("server_port")),
                    ),
                    "source_port": row.get(
                        "source_port",
                        row.get("src_port", row.get("client_port")),
                    ),
                    "first_flight_packet_size": row.get(
                        "first_flight_packet_size", row.get("first_flight_bytes")
                    ),
                }
                return guess_l7_protocol(data)
